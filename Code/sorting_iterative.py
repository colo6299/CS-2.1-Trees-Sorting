    ...
    while finished is False:
        ...
        for index, item in enumerate(item):
            if index is 0:
                continue
            if descending:
                if item > items[index-1]:  # What does DRY stand for, anyway?
...
def insertion_sort(items, descending=False):
    ...
    for index in range(1, len(items)):
        _insertion_helper(items, index, items[index], descending)

def _insertion_helper(list_in, sublist_index, item, descending=False):
    '''
    Handles insertion into sorted sublist during insertion sort. 

    sublist_index should equal working index.
    '''
    insert_index = _binary_locator(list_in, 0, sublist_index, item, descending)
...
def _binary_locator(list_in, lower_index, upper_index, item, descending=False):
    '''
